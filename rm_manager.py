...
def migrate_central_to_per_project(rm_manager_dir: str = None, rm_master_db_path: str = None) -> Dict:
    """NIC NIE ROBI — migracja jednorazowa, wykonana dawno temu.

    Przenosiła tabele etapów z centralnej bazy do baz per-projekt. Centralna
    baza od dawna ich nie ma (na serwerze też: 31 tabel, żadnej z project_*),
    a otwieranie jej jako pliku nie jest już możliwe. Zwraca wynik „nic do
    migracji" w formacie, którego oczekuje GUI.
    """
    return {'projects_migrated': 0, 'projects_skipped': 0,
            'errors': ['Brak tabeli project_stages w centralnej bazie - nic do migracji.'],
            'details': {}}


def cleanup_central_project_tables(rm_master_db_path: str = None, dry_run: bool = True) -> Dict:
    """NIC NIE ROBI — sprzątanie po migracji z `migrate_central_to_per_project`.

    Tabele projektowe w centralnej bazie już nie istnieją; DROP/VACUUM na
    bazie serwera nie jest operacją klienta. Wynik w formacie GUI.
    """
    return {'tables_found': [], 'table_stats': {}, 'tables_removed': [], 'dry_run': dry_run}
